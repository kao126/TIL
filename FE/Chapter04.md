...
- 開発ツール
  ...
  - 継続的インテグレーション支援ツール
    - ビルド：コンパイルやリンクを行って実行可能ファイルを作成すること
    - 継続的インテグレーション：共有の保管場所（リポジトリ）にマージするたびにビルドやテストを繰り返すこと
    - Jenkins：ビルドやテストを自動化するツール
    - デプロイ：実行可能ファイルを実行環境にインストールし、各種設定をして利用可能にすること
  - デバッグツール
...
- 形式言語：特定の目的のために人為的に作られた言語
  ...
  - 正規表現：文字列の集合規則を表す記法
- マークアップ言語：テキスト形式の文章にタグと呼ばれる特別な文字を使って、文章の構造や文字の修飾を記述する言語
  - SGML(Standard Generalized Markup Language)：電子的な文書の管理や交換を容易に行うためのマークアップ言語
  - HTML(Hyper Text Markup Language)：Webページを作成するためのマークアップ言語
    - **CSS**(Cascading Style Sheets)：文字の大きさ・文字の色・行間などの視覚表現
  - XML(eXtensible Markup Language)：ネットワークを介した情報システム間のデータ交換を容易にするためのマークアップ言語
    - DTD(Document Type Definition)：文字型定義
  - Ajax(Asynchronous JavaScript + XML)：XMLとJavaScriptがもつ非同期のHTTP通信機能を使い、動的に画像を再描画する仕組み

> 「HTML」とくれば「Webページを作成するためのマークアップ言語」
> 
> 「XML」とくれば「利用者独自のタグを定義できるマークアップ言語」
